...
import { ORGANIZATION_INVITE_EMAIL_COPY, type EmailLocale } from "./copy.js";
...
export function OrganizationInviteEmail({ locale, organizationName, acceptUrl }: OrganizationInviteEmailProps) {
  const copy = ORGANIZATION_INVITE_EMAIL_COPY[locale];

  return (
    <Html>
      <Head />
      <Preview>{copy.preview.replace("{organizationName}", organizationName)}</Preview>
      <Body style={{ backgroundColor: "#f4f4f5", fontFamily: "-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif", margin: 0, padding: "32px 16px" }}>
        <Container style={{ backgroundColor: "#ffffff", borderRadius: "16px", margin: "0 auto", maxWidth: "480px", padding: "40px 32px" }}>
          ...
          <Heading style={{ color: "#111827", fontSize: "22px", fontWeight: 700, lineHeight: 1.3, margin: "24px 0 16px" }}>
            {copy.heading.replace("{organizationName}", organizationName)}
          </Heading>
          <Text style={{ color: "#374151", fontSize: "15px", lineHeight: 1.6, margin: "0 0 24px" }}>
            {copy.body.replace("{organizationName}", organizationName)}
          </Text>
          <Button
            href={acceptUrl}
            style={{
